...
class baozimh():  #https://cn.baozimh.com/
    def output(importurl,import_list,choose):
        n = re.search(r'comic\/(.*?)$',importurl)
        comic_name = n.group(1)
        m = baozimh.getlist(importurl)
        for i in range(len(m)):
            print (f'{i+1}. {m[i][1]}')
        comic_input = input('输入章节序号：')
        print('请稍等...')
        comic_input_list = getfunction.region2list(comic_input)
        for i in range(len(comic_input_list)):
            comic_capter = comic_input_list[i] - 1
            ...
            getfunction.merge(final_list,directory_name)
        print('全部下载完毕')
